package org.springboot.workorder.util.excel;
...
import org.springboot.workorder.util.common.Common;
import org.springboot.workorder.util.common.PostfixTool;
...
public class ExcelReader {
	...
	public String readSheetName(String path,int num) throws IOException {
		if (path == null || Common.EMPTY.equals(path)) {
			return null;
		} else {
			String postfix = PostfixTool.getPostfix(path);
			if (!Common.EMPTY.equals(postfix)) {
				if (Common.OFFICE_EXCEL_2003_POSTFIX.equals(postfix)) {
					return readxlsSheetName(path,num);
				} else if (Common.OFFICE_EXCEL_2007_POSTFIX.equals(postfix)) {
					return readxlsxSheetName(path,num);
				}
			} else {
				logger.info(path + Common.NOT_EXCEL_FILE);
			}
	...
	public String readxlsSheetName(String path,int num) throws IOException {
		InputStream is = new FileInputStream(path);
		hssfWorkbook = new HSSFWorkbook(is);
		hssfSheet = hssfWorkbook.getSheetAt(num);
		return hssfSheet.getSheetName();
	}
	public String readxlsxSheetName(String path,int num) throws IOException {
		InputStream is = new FileInputStream(path);
		xssfWorkbook = new XSSFWorkbook(is);
		xssfSheet = xssfWorkbook.getSheetAt(num);
	...
	public static void main(String[] args) throws IOException{
		...
		List<Map<String,String>> lista = excelReader.readExcel("F:/BAK/temp/新建 FTP采集模板 - 样例.xlsx");
		System.out.println(excelReader.getHeads());
		for(@SuppressWarnings("rawtypes") Map map:lista){
